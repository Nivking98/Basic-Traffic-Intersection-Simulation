...
def repeat():
    ...
    while signals[current_green].green > 0:
        ...
        time.sleep(1)
    current_yellow = 1
    if current_green == 1 or current_green == 3:  # For 'D' and 'up' directions
        lane_range = 1
    else:
        lane_range = 3  # For 'B1B2' and 'A1A2' directions

    for i in range(lane_range):
...
def update_green_times():
    global green_default
    # Example: Randomly update green times within a range (e.g., 5 to 15 seconds)
    for i in range(no_of_signals):
        green_default[i] = random.randint(5, 15)
        signals[i].green = green_default[i]
    print("Updated green times:", green_default)
